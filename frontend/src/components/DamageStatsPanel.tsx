...
import { Bar, BarChart, ResponsiveContainer, XAxis, YAxis, Tooltip as RechartsTooltip, LabelList } from 'recharts';
...
const DamageStatsPanel = () => {
  ...
  const damageStats = boardChampions.map((champion) => {
    const stars = champion.stars || 1;
    const cost = champion.cost;
    // Calculate damage based on champion attributes
    const damage = Math.floor(cost * 100 * stars * (0.8 + Math.random() * 0.4));
            ...
            <div className="h-64 mb-4">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  >
                    <LabelList dataKey="damage" position="right" fill="#fff" />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
              ...
              {damageStats.map((stat, i) => (
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center">
                    <div className={cn("w-6 h-6 rounded-full flex items-center justify-center mr-2", 
