...
var _ = Describe("DynamicTimeItemSystem", func() {
    ...
    Context("Quicksilver", func() {
        ...
        It("should track immunity duration correctly and deactivate", func() {
            ...
            Expect(exists).To(BeFalse(), "QuicksilverEffect component should be removed after duration expires")
        })

        It("should stack Attack Speed correctly during immunity", func() {
            err := equipmentManager.AddItemToChampion(entity, "TFT_Item_Quicksilver")
            Expect(err).NotTo(HaveOccurred())

            // Time just before first proc
            simulateTime(dynamicTimeSystem, world, entity, procInterval-(deltaTime/2), deltaTime)
            effect, _ := world.GetQuicksilverEffect(entity)
            Expect(effect.GetCurrentBonusAS()).To(BeNumerically("~", 0.03))
            Expect(attackComp.GetBonusPercentAttackSpeed()).To(BeNumerically("~", 0.03))

            // Time just after first proc
            ...
            Expect(effect.GetProcTimer()).To(BeNumerically("~", deltaTime, 0.001)) // Timer resets with overflow

            // Time just after second proc
            simulateTime(dynamicTimeSystem, world, entity, procInterval-effect.GetProcTimer(), deltaTime) // Simulate remaining time
            ...
            simulateTime(dynamicTimeSystem, world, entity, 17.9, deltaTime) // Simulate remaining time
            effect, _ = world.GetQuicksilverEffect(entity)
            expectedStacks := 8 // floor(17.9 / 2) = 8
        ...
        It("should stop stacking Attack Speed after immunity expires", func() {
            ...
            _, exists := world.GetQuicksilverEffect(entity)
            Expect(exists).To(BeFalse()) // Component should be gone
